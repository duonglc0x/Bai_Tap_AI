...
| **DFS (Depth-First Search)** | Bản mô phỏng sử dụng tìm kiếm giới hạn độ sâu **DLS (Depth-Limited Search)** với `depth_limit = 8` để tránh nhánh vô hạn của DFS truyền thống trong không gian trạng thái cực lớn của 15-Puzzle. Thuật toán kết hợp lưu trữ độ sâu nhỏ nhất của các trạng thái đã đi qua nhằm tối ưu hóa và loại bỏ các nhánh lặp kém hiệu quả. |
| **UCS (Uniform Cost Search)** | Sử dụng hàng đợi ưu tiên **Priority Queue** để tìm kiếm đường đi có tổng chi phí $g(n)$ thấp nhất. Để mô phỏng rõ nét bản chất của UCS (khả năng xử lý các hành động có chi phí khác nhau, phân biệt với BFS), hệ thống quy định chi phí di chuyển Lên/Xuống là 1 và Trái/Phải là 2. UCS sẽ tìm đường đi tối ưu hóa tổng chi phí này thay vì chỉ tối ưu hóa số bước đi. |
...
| **Forward Checking (FC)** | Tương tự Backtracking kết hợp nhìn trước (look-ahead): Tại mỗi bước đi, thuật toán kiểm tra trước miền giá trị của bước tiếp theo. Nếu khoảng cách Manhattan của trạng thái tiếp theo vượt quá số bước còn lại cho phép (`nxt.manhattan() > depth_limit - (depth + 1)`), miền giá trị tương lai bị rỗng và thuật toán thực hiện **cắt nhánh sớm** ngay lập tức. |
